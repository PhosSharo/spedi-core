...
const routeRoutes = async (fastify) => {
    ...
    fastify.get('/routes', {
        ...
        const { device_id, status, limit, offset } = request.query;
        const parsedLimit = limit ? parseInt(limit, 10) : 50;
        const parsedOffset = offset ? parseInt(offset, 10) : 0;
        const result = await route_service_1.routeService.list({ device_id, status }, { limit: parsedLimit, offset: parsedOffset });
        return result;
    });
    /**
     * POST /routes
    ...
    fastify.post('/routes', {
        ...
        const { device_id, name, waypoints } = request.body;
        if (!device_id || !name) {
        ...
        try {
            const route = await route_service_1.routeService.create(device_id, request.user.id, name, waypoints);
    ...
    fastify.get('/routes/:id', {
        ...
        const route = await route_service_1.routeService.getById(id);
        if (!route) {
    ...
    fastify.delete('/routes/:id', {
        ...
        const { id } = request.params;
        try {
            await route_service_1.routeService.deleteDraft(id, request.user.id);
    ...
    fastify.post('/routes/:id/start', {
        ...
        const { id } = request.params;
        try {
            const route = await route_service_1.routeService.dispatch(id, request.user.id);
    ...
    fastify.post('/routes/:id/stop', {
        ...
        const { id } = request.params;
        try {
            await route_service_1.routeService.abort(id, request.user.id);
